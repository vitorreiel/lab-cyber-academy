#!/bin/bash
dest="/opt/labs/laboratory-02/cyrm-attack"
...
cd /opt/labs/laboratory-02/cyrm-attack && docker run -d --name containernet -it --rm --privileged --pid='host' -v /var/run/docker.sock:/var/run/docker.sock --mount type=bind,source="${dest}/topology",target=/cyrm-attack containernet-cyrm python3 /cyrm-attack/corporate-attack-scenario.py
